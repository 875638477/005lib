...
### 5.4.1 输入、输出、SPI 各自干什么

下载器对外仍是 SWD：一根时钟 SWCLK，一根数据线 SWDIO。SWDIO 要轮流说话。

一次读寄存器大致是：

```text
下载器说出 8 位请求     ← 这时下载器是输出
空 1 个时钟             ← 换方向
目标回答 3 位 ACK       ← 这时下载器是输入
目标送出 32 位数据      ← 还是输入
再换方向，下载器收回线
```

| 名字 | 谁在驱动那根线 | HSLink 用哪只脚 |
| --- | --- | --- |
| 输出 | 下载器把 0/1 推给目标 | PA29 |
| 输入 | 目标把 0/1 送回来，下载器只看 | PA28 |

两只脚在板子上焊在一起，接到同一根 SWDIO。对外还是一根线。

只用一只脚也能做：说话时把它设成输出，听话时改成输入。每次换方向都要 CPU 改寄存器，80 MHz 时来不及。两只脚分开后，PA29 负责推，不用时放开；PA28 一直听着。

这里的 SPI 不接 SPI Flash，也不跟目标做 SPI 通信。5301 里的 SPI1 外设只被当成一个硬件移位器：

| SPI1 脚 | 接到 | 实际含义 |
| --- | --- | --- |
| SCLK | SWCLK | 调试时钟，由下载器产生 |
| MOSI | PA29，输出 | 把要发出的位按时钟推出去 |
| MISO | PA28，输入 | 按同一时钟把线上的位收进来 |

CPU 把“请求”放进 SPI 发送寄存器，SPI 自己按固定时钟一位一位送出，同时把线上的回答收进接收寄存器。GPIO 一位一位翻转做不到这么稳、这么快。板上那片 W25Q 用的是另一组 SPI2，和这条 SWD 无关。

[HSLink Pro](https://cherrydap.cherry-embedded.org/projects/HSLink%20Pro.html) 的 20pin 上仍然只有**一根** TMS/SWDIO。并联发生在下载器内部。
