...
## 2.5 电感电流：怎么得到？——测，不是靠占空比“推算”

**快环里的 \(I_L\) / IPH：必须实测。**  
用 \(V,L,D,f_{sw}\) 的公式只适合 **选型估算纹波**（§2.2），不能代替闭环反馈。

### 两种主流测法

| 方式 | 做法 | 优点 | 缺点 | 本方案 |
|------|------|------|------|--------|
| **采样电阻（分流）+ 放大器** | 功率回路串 \(R_{sense}\)（mΩ 级），INA240 等放大 \(I\cdot R\) 后进 ADC | 成本低、带宽高、线性好、易逐周期限流 | 有 \(I^2R\) 损耗；要 Kelvin；大电流要极小阻值 | **IPH / IOUT 快环推荐** |
| **电流传感器（霍尔 / 磁通门等）** | 导线穿过或贴母排，输出模拟/数字电流 | 隔离好、几乎不增导通损耗 | 贵、零点/温漂、带宽与延迟往往不如分流+INA | 特大电流或必须隔离时可选用 |

你前面说的 INA240 / INA229：**都不是“凭空读电感”**，而是读 **分流电阻上的电压**，再换算成电流：

\[
I_L = \frac{V_{\mathrm{shunt}}}{R_{\mathrm{sense}}}
\quad\Rightarrow\quad
\text{经 INA 增益后}\quad
I_L = \frac{V_{\mathrm{adc}}-V_{\mathrm{ref}}}{G\cdot R_{\mathrm{sense}}}
\]

所以：

- **“用传感器读”**——若指霍尔模块：可以，但是另一条技术路线  
- **“用采样电阻读”**——本方案默认；INA240/INA229 是放大器/数字监视器，**前面仍要有 \(R_{sense}\)**  
- **“纯公式计算电感电流”**——只用于设计 L、估 \(\Delta I_L\)，**不做均流/OCP 主反馈**

### 和“直接测电感”的关系

没有磁芯抽头时，工程上就是测 **与电感串联的那一段回路电流**（电感电流 = 该支路电流）。  
取样点仍在 L 的回路上（或该相低端回流），测到的就是 \(I_L\)。
